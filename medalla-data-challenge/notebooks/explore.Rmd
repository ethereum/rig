...
Let's first count how many aggregates are strongly clashing.
...
| Subset aggregate | _If not strongly redundant_, an aggregate fully contained in another aggregate included _in the same block_ | There are `r n_subset_ats` subset aggregates until slot 25000, `r percent_subset`% of all aggregates until slot 25000 | Drop all subset aggregates |
| Strongly clashing aggregates | _If not a subset aggregate_, an aggregate with attesting indices $I$ such that there exists another aggregate _attesting for the same in the same block_ with attesting indices $J$ and $I \cap J \neq \emptyset$ | There are `r n_strongly_clashing` strongly clashing aggregates until slot 25000, `r percent_strongly_clashing`% of all aggregates until slot 25000 | These cannot be aggregated further. Do nothing |
| Weakly clashing aggregates | _If not a strongly clashing aggregate_, an aggregate with attesting indices $I$ such that there exists another aggregate _attesting for the same in the same block_ with attesting indices $J$ | There are `r n_weakly_clashing` weakly clashing aggregates until slot 25000, `r percent_weakly_clashing`% of all aggregates until slot 25000 | These can be aggregated further into one aggregate with attesting indices $I \cup J$. In an ideal world, we have 0 weakly clashing aggregates |
:::
...
| Name | Explanation | Statistics | Recommendation |
|-|-|-|-|
| Subset aggregate of size 1 | A subset aggregate which is an unaggregated individual attestation | There are `r n_subset_ind_ats` subset aggregates of size 1 until slot 25000, `r percent_subset_ind`% of all subset aggregates until slot 25000 | Definitely drop these |
