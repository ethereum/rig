...
stats_per_slot <- get_stats_per_slot(all_ats, committees)
...
```{r}
blocks_with_conflicting_clients <- all_bxs[declared_client != "undecided"] %>%
...
We look at data between epochs 0 and `r end_epoch`.
...
all_ats %>%
  ...
  merge(validators[, .(validator_index, client)],
        by.x = c("proposer_index"), by.y = c("validator_index")) %>%
  ggplot(aes(x = client, y = included_ats)) +
  stat_summary(geom = "bar", fun = mean, fill = myred) +
...
```{r}
get_attestation_grid <- function(start_epoch, end_epoch, committees = NULL) {
  ...
  attestation_grid <- all_ats %>%
    ...
    merge(validators[, .(validator_index, declared_client=client)]) %>%
    unique()
}

get_grid_per_client <- function(start_epoch, end_epoch, client, attestation_grid) {
...
plot_grid <- function(start_epoch, end_epoch, committees = NULL) {
  attestation_grid <- get_attestation_grid(start_epoch, end_epoch, committees)
  
  l = list()

  for (client in c("prysm", "lighthouse", "nimbus", "teku")) {
    l[[client]] <- get_grid_per_client(start_epoch, end_epoch, client, attestation_grid)
  }
...
attestation_grid <- get_attestation_grid(start_epoch, end_epoch, committees = committees)
```

#### Lighthouse

```{r, layout="l-screen", fig.height=2}
get_grid_per_client(start_epoch, end_epoch, "lighthouse", attestation_grid)
```
#### Nimbus

```{r, layout="l-screen", fig.height=2}
get_grid_per_client(start_epoch, end_epoch, "nimbus", attestation_grid)
```

#### Prysm

```{r, layout="l-screen", fig.height=2}
get_grid_per_client(start_epoch, end_epoch, "prysm", attestation_grid)
```

#### Teku

```{r, layout="l-screen", fig.height=2}
get_grid_per_client(start_epoch, end_epoch, "teku", attestation_grid)
```
...
all_bxs %>%
  ...
  ggplot(aes(x = client, y = n_subset)) +
  stat_summary(geom = "bar", fun = mean, fill = myred) +
  stat_summary(geom = "errorbar", fun.data = mean_se, width = 0.2) +
