...
block_root_at_slot <- get_block_root_at_slot(fread(here::here("mainnet_data/all_bxs.csv")))
all_ats <- fread(here::here("mainnet_data/all_ats.csv"))
committees <- fread(here::here("mainnet_data/committees.csv"))
...
new_val_series <- get_stats_per_val(
  ...
  block_root_at_slot, committees = committees, chunk_size = 10)
list(val_series, new_val_series) %>% rbindlist() %>% fwrite(here::here("mainnet_data/val_series.csv"))
rm(new_val_series)
...
print(str_c("Max slot ", (end_epoch+1)*slots_per_epoch - 1))
```
...
We look at data between epochs 0 and `r end_epoch` (`r get_date_from_epoch(end_epoch)`) and report updated metrics for eth2's mainnet. You can also find a similar notebook for [Medalla here](https://ethereum.github.io/rig/eth2data/notebooks/medalla_explore.html) and [Pyrmont here](https://ethereum.github.io/rig/eth2data/notebooks/pyrmont_explore.html).
...
| Aggregate of size 1 | An attestation included without being aggregated | There are `r n_size_1_ags` aggregates of size 1, `r percent_size_one`% of all aggregates until slot `r subset_until_slot` | Either it is weakly clashing, so aggregate it further; or it is a subset aggregate, so drop it; or it is a redundant, drop it; or it is new and never aggregated, keep it |
:::