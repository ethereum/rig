...
```{r}
total_gas_spent <- (blocks %>%
  summarise(total_gas = sum(gas_used)))$total_gas[1]

empirical_bid_distribution <- txs %>%
  group_by(gas_price) %>%
  summarise(freq = sum(gas) / total_gas_spent) %>%
  add_column(cumfreq = cumsum(.$freq))

txs %>%
  group_by(block_number) %>%
  summarise(gas_used = sum(gas))

stats_per_chunk <- blocks_sample %>%
  mutate(block_chunk = block_number %/% chunk_size) %>%
  replace_na(list(
    avg_gas_price = 0, gas_used = 0)) %>%
  mutate(block_num = gas_used * avg_gas_price) %>%
...
lookup <- tibble(
  prices = stats_per_chunk$avg_gas_price,
  indices = findInterval(stats_per_chunk$avg_gas_price * (10 ** 9), empirical_bid_distribution$gas_price)
) %>%
  add_column(q = empirical_bid_distribution$cumfreq[.$indices])
...
```{r}
txs %>%
  filter(hash == "0xb2eef29aa037f064acb08eb520343ad67aa3f42661d53779c8244b904a9c1801")
```

```{r}
txs %>%
  filter(hash == "0xc26bb4efa05c6d6a75c78c83ce3a0f457b834b3da2560260ce536d04e0e4bb68")
```
