...
While real world gallons of oil went negative, Ethereum gas prices have sustained a long period of high fees since the beginning of May. I wanted to dig in a bit deeper, with a view to understanding the fundamentals of the demand. Some of the charts below retrace steps that are very well-known to a lot of us -- these are mere restatements and updates.

For instance, Onur Solmaz from Casper Labs wrote [a very nice post](https://solmaz.io/2019/10/21/gas-price-fee-volatility/) arguing that since we observe daily cycles, there must be something more than one-off ICOs and Ponzis at play.
...
We will see them here too, and a few more questions I thought were interesting (or at least, that I kinda knew the answer to but never had derived or played with myself). This is an excuse to play with my new Dappnode full node, using the wonderful [ethereum-etl](https://github.com/blockchain-etl/ethereum-etl) package from Evgeny Medvedev to extract transaction and block details.
...
blocks %>%
  ggplot() +
  geom_histogram(aes(x = gas_used), bins = 1000) +
  scale_y_log10()
```
...
blocks_sample %>%
  ...
  ggplot() +
  geom_line(aes(x = block_date, y = ma_prop_used))
```

Where does the dip on May 15th come from?
...
blocks %>%
  ggplot() +
  geom_point(aes(x = gas_used, y = size)) +
  scale_y_log10() +
...
blocks_sample %>%
  ...
  ggplot() +
  geom_line(aes(x = block_date, y = avg_gas_price), colour = "red")
```
...
blocks %>%
  ...
  ggplot() +
  geom_histogram(aes(x = time_difference), binwidth = 1)
```
