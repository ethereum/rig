...
def get_specs(constants):
    ...
    def process_slot(state: BeaconState) -> None:
        # Cache state root
        # hash_tree_root(state) is the bottleneck here in terms of speed
        # (~ 100x slower than hash_tree_root(state.latest_block_header))
    ...
    def process_justification_and_finalization(state: BeaconState) -> None:
        ...
        print("previous epoch validator weight", (float(get_attesting_balance(state, matching_target_attestations)) / get_total_active_balance(state)) * 100, "%")
        ...
        print("current epoch validator weight", (float(get_attesting_balance(state, matching_target_attestations)) / get_total_active_balance(state)) * 100, "%")
        if get_attesting_balance(state, matching_target_attestations) * 3 >= get_total_active_balance(state) * 2:
    ...
    def get_attestation_deltas(state: BeaconState) -> Tuple[Sequence[Gwei], Sequence[Gwei]]:
        ...
        if finality_delay > MIN_EPOCHS_TO_INACTIVITY_PENALTY:
            print("inactivity penalty kicking in")
    ...
    def process_rewards_and_penalties(state: BeaconState) -> None:
        if get_current_epoch(state) == GENESIS_EPOCH:
            print("not processing rewards_and_penalties")
            return
    ...
    class Specs():
        def __init__(self):
            ...
            self.bls_verify = bls_verify
            self.aggregate_pubkeys = aggregate_pubkeys
            self.is_valid_merkle_branch = is_valid_merkle_branch
            self.integer_squareroot = integer_squareroot
            self.xor = xor
            self.int_to_bytes = int_to_bytes
            self.bytes_to_int = bytes_to_int
            self.is_active_validator = is_active_validator
            self.is_slashable_validator = is_slashable_validator
            self.is_slashable_attestation_data = is_slashable_attestation_data
            self.is_valid_indexed_attestation = is_valid_indexed_attestation
            self.compute_shuffled_index = compute_shuffled_index
            self.compute_proposer_index = compute_proposer_index
            self.compute_committee = compute_committee
            self.compute_epoch_at_slot = compute_epoch_at_slot
            self.compute_start_slot_at_epoch = compute_start_slot_at_epoch
            self.compute_activation_exit_epoch = compute_activation_exit_epoch
            self.compute_domain = compute_domain
            self.get_current_epoch = get_current_epoch
            self.get_previous_epoch = get_previous_epoch
            self.get_block_root = get_block_root
            self.get_block_root_at_slot = get_block_root_at_slot
            self.get_randao_mix = get_randao_mix
            self.get_active_validator_indices = get_active_validator_indices
            self.get_validator_churn_limit = get_validator_churn_limit
            self.get_seed = get_seed
            self.get_committee_count_at_slot = get_committee_count_at_slot
            self.get_beacon_committee = get_beacon_committee
            self.get_beacon_proposer_index = get_beacon_proposer_index
            self.get_total_balance = get_total_balance
            self.get_total_active_balance = get_total_active_balance
            self.get_domain = get_domain
            self.get_indexed_attestation = get_indexed_attestation
            self.get_attesting_indices = get_attesting_indices
            self.increase_balance = increase_balance
            self.decrease_balance = decrease_balance
            self.initiate_validator_exit = initiate_validator_exit
            self.slash_validator = slash_validator
            self.initialize_beacon_state_from_eth1 = initialize_beacon_state_from_eth1
            self.is_valid_genesis_state = is_valid_genesis_state
            self.state_transition = state_transition
            self.verify_block_signature = verify_block_signature
            self.process_slots = process_slots
            self.process_slot = process_slot
            self.process_epoch = process_epoch
            self.get_matching_source_attestations = get_matching_source_attestations
            self.get_matching_target_attestations = get_matching_target_attestations
            self.get_matching_head_attestations = get_matching_head_attestations
            self.get_unslashed_attesting_indices = get_unslashed_attesting_indices
            self.get_attesting_balance = get_attesting_balance
            self.process_justification_and_finalization = process_justification_and_finalization
            self.get_base_reward = get_base_reward
            self.get_attestation_deltas = get_attestation_deltas
            self.process_rewards_and_penalties = process_rewards_and_penalties
            self.process_registry_updates = process_registry_updates
            self.process_slashings = process_slashings
            self.process_final_updates = process_final_updates
            self.process_block = process_block
            self.process_block_header = process_block_header
            self.process_randao = process_randao
            self.process_eth1_data = process_eth1_data
            self.process_operations = process_operations
            self.process_proposer_slashing = process_proposer_slashing
            self.process_attester_slashing = process_attester_slashing
            self.process_attestation = process_attestation
            self.process_deposit = process_deposit
            self.process_voluntary_exit = process_voluntary_exit
            self.get_committee_assignment = get_committee_assignment
